<script>
    ...
    let trials = [{forma1: '', forma2: '', forma3: '', frase: ''}];
    
    let j = 0;
    let scrittaEsattezza = "";
    let scritta = "";
    let risposta;
    let risultati = [];
    ...
    const premutoA = () => {
        ...
        if (risposta == trials[j].corretto) {
            console.log("Esatto");
            scrittaEsattezza = "Esatto";
            risultati.push(true);
            transition(true);
        }else if (risposta != trials[j].corretto) {
            console.log("Sbagliato");
            scrittaEsattezza = "Sbagliato";
    ...
    $: {       
        if(j == n){
            ...
            if(risultati[risultati.length-1]){
                setTimeout(() => {$attivo = 'InizioTest'}, ritardo);
            }else{
                setTimeout(() => {$attivo = 'Invalido'}, ritardo);
            }
